        ...
        <div class="mx-auto max-w-7xl space-y-8 sm:px-6 lg:px-8">
            @foreach($datas as $data)
                <div class="overflow-hidden bg-white rounded-md border p-5 shadow">
                    <h3><a href="{{ route('posts.show', ['post' => $data->slug])}}" class="text-blue-500">{{$data->title}}</a></h3>
                    <div class="mt-4 flex justify-between">
                        ...
                        <div class="flex gap-2 items-center">
                            ...
                            <span class="text-gray-500">{{ \Carbon\Carbon::parse($data->published_at)->format('Y-m-d') }}</span>
                        </div>
                    </div>
                </div>
            @endforeach
